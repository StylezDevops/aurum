...
The full architecture — every organ, its problem, build, interface, gate, and acceptance
test — lives in **[`aurum_organs_spec.md`](./aurum_organs_spec.md)**. Current build status is
tracked in **[`progress.md`](./progress.md)**. This README is the map; the spec is the territory.
...
The architecture is organised into tiers in [`aurum_organs_spec.md`](./aurum_organs_spec.md).
**Tier 0 (the spine) is built and tested**; the rest is specified and is the active roadmap.
...
### 🧭 Designed — the roadmap (`aurum_organs_spec.md`)
...
Each carries a build-confidence label in the spec (HIGH / PARTIAL / HARDEST) — an honest
...
These hold across every organ (full list in the spec):
...
| Path | Purpose |
|------|---------|
| [`aurum_organs_spec.md`](./aurum_organs_spec.md) | The full organ architecture (build spec) |
